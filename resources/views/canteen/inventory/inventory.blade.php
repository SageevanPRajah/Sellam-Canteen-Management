<x-app-layout>
    <h1>Inventory for Show on {{ $show->date }} at {{ $show->time }}</h1>
    <form action="{{ route('canteen.inventory.update', $show->id) }}" method="POST">
        ...
        <!-- Two buttons: one for updating stock and one for updating inventory (and recording income) -->
        <button type="submit" name="action" value="update_stock">Update Stock</button>
        <button type="submit" name="action" value="update_inventory">Update Inventory</button>
    </form>
</x-app-layout>
